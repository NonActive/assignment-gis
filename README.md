...
After users click on the map within the Prague, the basic information about the location is shown.

The user can also select a city district from the table, that is shown on the map.
...
**Get a noise map for the city of Prague**

`GET api/noise-map`

Response:
-  FeatureCollection GeoJSON - each feature contains geometry and properties with additional information
```json
{
	...
	"features" : [{ 
		"type" : "Feature", 
		"properties" : {  
			"level" : "12.5", # noise level
			"max" : "72.5", # max value of noise
			"min" : "2.5", # min value of noise
		}, 
		"geometry" : { 
			"type" : "MultiPolygon", 
			"coordinates" : [] 
...
**Get an air quality map for the city of Prague**
...
```json
{
	...
	"features" : [{ 
		"type" : "Feature", 
		"properties" : {  
			"gridvalue" : "12.5", # level of air quality
			"max" : "5", # max level of air quality
			"min" : "1", # min level of air quality
		}, 
		"geometry" : { 
			"type" : "MultiPolygon", 
			"coordinates" : [...] 
...
**Get a price map for the concrete city district**
...
```json
{
	...
	"features" : [{ 
		...
		"properties" : {  
			"price" : "123.5", 
			"max" : "1238.5", # maximal price in district
			"min" : "20.5", # minimal price in district
		}, 
		"geometry" : { 
			"type" : "MultiPolygon", 
			"coordinates" : [...] 
...
**Get a data for the concrete city district**

`GET api/city-zone/1`

Response:
-  Feature GeoJSON - feature contains geometry and properties with additional information
```json
{
  "type" : "Feature", 
  "properties" : {  
...
**Get an information about specific point**

`GET api/point-info?point[]=lng&point[]=lat`

Response:
 - JSON contains  2 top-level keys, point-info and parks. Parks contain an array of green leisure areas within the near region and point-info contains GeoJSON Feature about land availability. Attributes of the park are mostly self-evident.
```json
{
   "point-info":{
      "type":"Feature",
      "properties":{
         "zastavitelne":"nezastavitelne", # land availability
         "price":null, 
         "noise":42.5,
         "air_quality":2
      },
...
**Get an overview information for all city districts**

`GET api/city-zone`

Response:
- an array of objects containing overview information about districts
```json
[
   {
      "gid":1,
